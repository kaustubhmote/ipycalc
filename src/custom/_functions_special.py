...
def redor(dc, time=None, npts=None):
    ...
    if npts is None:
        ...
        if not isinstance(npts, int):
            raise ValueError(f"Number of points must be an integer, not {npts} which belongs to {type(npts)}")

    if time is None:
        time = np.linspace(1e-10, 10 / dc, npts)
    else:
        time = np.linspace(1e-10, time, npts)
